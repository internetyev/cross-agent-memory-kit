...
DEFAULT_STATE_PATH = STATE_ROOT / "cursor-processed.json"
DEFAULT_LOG_PATH = STATE_ROOT / "logs" / "cursor.log"
DEFAULT_SESSION_GLOBS = [
    Path.home() / ".cursor" / "projects" / "**" / "agent-transcripts" / "**" / "*.jsonl",
]
...
def main() -> None:
    ...
    for path in candidates:
        ...
        try:
            transcript = load_cursor_transcript(path)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scan Cursor session JSONL files and distill unprocessed sessions.")
    parser.add_argument("--state-path", type=Path, default=DEFAULT_STATE_PATH)
    parser.add_argument("--log-path", type=Path, default=DEFAULT_LOG_PATH)
    parser.add_argument("--quiet-minutes", type=int, default=30)
    parser.add_argument("--lookback-days", type=int, default=7)
    parser.add_argument("--limit", type=int, default=5)
    parser.add_argument("--provider", default="cursor-cli")
    ...
    parser.add_argument(
        "--mark-existing",
        action="store_true",
        help="Mark matching sessions as baseline without distilling them.",
    )
