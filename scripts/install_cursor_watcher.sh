#!/usr/bin/env bash
# Install the macOS launchd job that distills quiet Cursor sessions daily at 04:00.
...
PY="$VENV/bin/python"
SCANNER="$REPO_ROOT/wrappers/cursor_session_scan.py"
LABEL="com.mcp-memory.cursor-distill"
TEMPLATE="$REPO_ROOT/launchd/memory-distill.plist.template"
PLIST_DST="$HOME/Library/LaunchAgents/$LABEL.plist"
STATE_ROOT="$HOME/.local/state/cross-agent-memory-kit"
LOG_OUT="$STATE_ROOT/logs/cursor-launchd.out.log"
LOG_ERR="$STATE_ROOT/logs/cursor-launchd.err.log"
...
# Mark all currently-quiet Cursor sessions as baseline so the first run does not
...
echo "Installed Cursor memory distillation watcher:"
echo "  $PLIST_DST"
echo
echo "Logs:"
echo "  $STATE_ROOT/logs/cursor.log"
echo "  $LOG_OUT"
echo "  $LOG_ERR"
